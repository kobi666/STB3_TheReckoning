...
namespace BansheeGz.BGSpline.Components
{
    ...
    public abstract class BGCcCustomColliderAbstract<T> : BGCcSplitterPolyline, BGCcColliderI where T : Component
    {
                    ...
                    for (var i = 1; i < count; i++)
                    {
                        ...
                        if (workingList.Count <= cursor)
                        {
                            if (childPrefab == null)
                            {
                                ...
                                ChildCollisionDetector collisionDetector = go.GetOrAddComponent<ChildCollisionDetector>();
                                collisionDetector.InitChildDetector(ParentMyGameObject,CollisionAggregator,CollisionAggregator.TagsICanDetect, CollisionAggregator.ChildDetectablesTag,GoCounter);
                                GoCounter++;
                            }
                            else
                            {
